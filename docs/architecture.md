...
| `test:daily-stats-in-flight-result-application` | `scripts/tests/architecture/test-daily-stats-in-flight-result-application.mjs` | Fetch-audit Phase 1 regression fix: `useProfileSharedDailyStats`'s fetch effect has no `cancelled`-closure/cleanup pair (a same-context effect re-run — triggered independently by `contextKey` resolving or a `request()`/`retry()`/invalidation version bump — used to run the previous invocation's cleanup and discard an in-flight fetch's own valid, already-succeeding result, leaving `dailyStatsStatus`/`vocabularyGrowthStatus` stuck on `"loading"` forever even though the network request itself succeeded); result application (ready, error, and the no-session idle reset) is instead gated on `inFlightKeyRef.current` still matching `contextKey`, re-checked fresh at resolution time, and the pre-existing in-flight dedup guard that stops a second fetch from starting stays intact |
