...
| `scripts/generation/generate-sitemap.mjs` (`collectVocabularyRoutes`) | `getAllLocalizedVocabularyRoutes()` via `scripts/lib/load-vocabulary-level-routes.mjs` | `src/data/seo/vocabularyLevels/vocabularyLevelRoutes.ts` (route registry — the sole source of CEFR sitemap routes; the legacy `{ui}/{target}.json` matrix was deleted Phase 7, see `docs/generated-data.md`) | build-time generator | low |
