...
| `src/data/seo/verbLists/conjugated100Verbs/conjugated100VerbsContent.json` | handwritten source | itself | manual (no generator) | `conjugated100VerbRegistry.ts`, `ConjugatedVerbFormsSeoPage.tsx`, `src/seo/verbLists/conjugated100Verbs/conjugatedVerbFormsMetadata.ts`, `src/entry-server.tsx` (prerender), `scripts/generation/generate-sitemap.mjs` once launch-gated on | yes | medium — authored per target/UI-language combination |
