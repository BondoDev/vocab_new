...
| `src/data/seo/verbLists/conjugated100Verbs/textContent/` | handwritten source, **no generator exists** | itself (hand-authored) | none found | client + SSR (`conjugated100VerbRegistry.ts`, eager glob), transitively `ConjugatedVerbFormsSeoPage.tsx`, `src/seo/verbLists/conjugated100Verbs/conjugatedVerbFormsMetadata.ts`, `src/entry-server.tsx` (prerender); `scripts/generation/generate-sitemap.mjs` reads it directly off disk (see `scripts/lib/load-conjugated-verb-forms-registry.mjs`), not through the glob | yes | medium — one file per target language (`{targetLanguage}.json`, e.g. `english.json`), each holding all `uiLanguage` variants of that target language's SEO copy; authored per target/UI-language combination |
| `src/data/seo/verbLists/conjugated100Verbs/conjugatedVerbs/` | handwritten source, **no generator exists** | itself (hand-authored) | none found | client + SSR (`conjugated100VerbFormsData.ts`, eager glob), transitively `ConjugatedVerbFormsTableSection.tsx` | yes | medium — one file per target language (`{targetLanguage}.json`, e.g. `english.json`), each holding that language's tense-grouped 100-verb conjugation rows; added by hand over time — a target language with no file simply renders the table placeholder (`getConjugatedVerbFormsRowsById` returns `null`) |
...
| Directory | Command |
|---|---|
| `src/data/seo/wordPages/word-hub-pages/`, `src/data/seo/wordPages/word-browse-shards/`, `src/data/seo/verbLists/shared/common100VerbLookup/` | `npm run generate:word-hub-data` |
