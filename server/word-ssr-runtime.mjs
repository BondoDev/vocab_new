...
function stripManagedHeadTags(template) {
  return template
    ...
    // same template (defensive — server-build/ssr-template.html is never
    // rendered with these baked in). Both current-format ids and the old
    // executable `window.X=...` shape are matched so a fresh render over a
    // stale intermediate template can't leave either behind.
