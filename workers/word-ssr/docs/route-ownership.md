...
| Worker browse-shard JSON | `/staging-assets/browse-shard/english/a1.json` | Not a static file — served from the same shard store as word pages; no current in-repository HTTP consumer is known (see note below) |
| Direct record-shard access | `/records`, `/records/latest/manifest.json`, `/records/{version}/concepts/english/a1.json` | Deliberately routed to the Worker via a selective `assets.run_worker_first = ["/records/*"]` (unlike every other row in this table, which reaches the Worker because no static file matches). The Worker's `fetch` handler returns a minimal `404` for any `/records` or `/records/*` request without calling `env.ASSETS.fetch()` for it — see `docs/deployment.md` for the full rationale. This is a separate mechanism from Worker-internal shard reads: `env.ASSETS.fetch()` calls made by the Worker's own code (manifest/shard loading) go directly to the `ASSETS` binding and are not routed through this denial branch. |

**Browse-shard route note:** the underlying browse-shard data
(generated by `generate-full-corpus.mjs`) remains required internally by
word-page SSR — `handleWordPageRequest` reads it for browse pagination
and related/discovery word resolution regardless of this route's status.
This separate public route exposes the same generated data over HTTP;
no current in-repository HTTP consumer of it is known. Current
client-side browse search uses a separate bundled implementation
(`src/data/seo/wordPages/word-browse-shards/`, loaded via
`wordBrowseSearchData.ts`) and does not call this route. The route is
retained pending production request-usage evidence — do not remove or
rename it without first checking operational traffic and compatibility
risk.
...
`run_worker_first` configuration is needed for that split; it falls out
naturally from what does/doesn't exist as a file in the assets directory.
`/records/*` is the one exception: those paths **do** exist as physical
files in `assets-full/`, so reaching the Worker requires the explicit
selective `run_worker_first = ["/records/*"]` rule described above — it is
scoped to that one prefix and does not change asset-first behavior for
anything else.
