...
async function main() {
  ...
  await test("failed Supabase write surfaces the real error message and does not call the storage write", async () => {
    let storageCalls = 0;
    const result = await saveAccountLanguagePair({
      buildPatchedProfile: () => ({ nativeLanguage: "en", practiceLanguage: "es" }),
  ...
  await test("failure with a non-Error throw still surfaces a clear, non-empty error message (no silent swallow)", async () => {
    const result = await saveAccountLanguagePair({
      buildPatchedProfile: () => ({ nativeLanguage: "en", practiceLanguage: "es" }),
      writeSupabaseUserProfile: async () => {
        throw "boom";
  ...
  await test("failed save path surfaces the error and leaves the popup open (no setIsOpen(false), no proceed())", () => {
    ...
    assert.match(failureBranch[1], /setSaveError\(result\.error\)/);
    assert.doesNotMatch(failureBranch[1], /setIsOpen\(false\)/);
    assert.doesNotMatch(failureBranch[1], /proceed\(\)/);
  });

  console.log("\n[6] Onboarding prefill/save wiring stays intact (untouched by this task)");
  ...
  // in the file header. Neither file was modified by this change; these are
  // source-text presence checks proving the existing contract is still
  // there, not a re-test of logic this task does not own.
