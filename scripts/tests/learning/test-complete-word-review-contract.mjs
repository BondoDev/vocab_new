...
test("2. The RPC request body sends exactly p_event_id, p_word_progress_id, p_result, p_stat_date", () => {
  ...
  assert.deepEqual(
    sentKeys.sort(),
    ["p_event_id", "p_result", "p_stat_date", "p_word_progress_id"].sort(),
...
test("4. completeWordReview's call site in ReviewSession.tsx passes only session/eventId/wordProgressId/result/statDateISO", () => {
  ...
  assert.deepEqual(
    passedKeys.sort(),
    ["eventId", "result", "session", "statDateISO", "wordProgressId"].sort(),
